...
namespace NexLIMS.API.Controllers
{
    ...
    public class AdmanEnrollController : ControllerBase
    {
        ...
        public AdmanEnrollController(ISignupService signupService, IHttpClientFactory httpClientFactory, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> Signup([FromBody] RegisterDto request)
        {
            ...
            var response = await client.PostAsJsonAsync(
                $"{_baseUrl}/api/payment/create-invoice",
                payload);
